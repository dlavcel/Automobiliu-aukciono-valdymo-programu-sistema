...
def looks_registered(scraper) -> bool:
    try:
        r = scraper.get(
            "https://autohelperbot.com/en/profile",
            headers=HEADERS,
        ...
        return "logout" in low or "orders" in low or "my account" in low
    except Exception:
        return False
...
def register_account(proxy_url: str, email: str, site_password: str, first_name: str, last_name: str):
    ...
    if not looks_registered(scraper):
