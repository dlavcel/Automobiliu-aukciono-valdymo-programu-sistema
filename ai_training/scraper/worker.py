...
from proxy_login import (
    create_authenticated_scraper,
    HEADERS,
    LoginError,
)
...
def reset_scraper():
    ...
    if hasattr(thread_local, "pace_list"):
        del thread_local.pace_list
    if hasattr(thread_local, "pace_detail"):
        del thread_local.pace_detail

def close_scraper_session(scraper):
    if scraper is None:
        return
...
def logout_scraper(config: dict):
    ...
    try:
        ...
        log(
            f"[AUTH][LOGOUT] worker={config['worker_name']} "
            f"status={r.status_code} url={r.url}"
        )
    except Exception as e:
        log(f"[AUTH][LOGOUT_ERR] worker={config['worker_name']} err={e}")

    close_scraper_session(scraper)
    reset_scraper()
...
def is_blocked_or_challenge_html(html: str) -> bool:
    low = (html or "").lower()
    ...
    for row in rows:
        cur.execute("""
            INSERT INTO list_results (
                detail_url, vin, title, sold_price, seller_type, engine,
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                fail_list_page(page, str(e), reset_to_pending=True)
                log(f"[LIST][AUTH] {worker_name}: session expired page={page}")

                try:
                    logout_scraper(config)
                except Exception:
                    reset_scraper()
